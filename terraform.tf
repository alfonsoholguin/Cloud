...
# Red Virtual
resource "azurerm_virtual_network" "net_rtual" {
  name                = "network_vm"
...
resource "azurerm_subnet" "test_cloudazure" {
  name                 = "SubNet_private"
  resource_group_name  = azurerm_resource_group.test_cloudazure.name
  virtual_network_name = azurerm_virtual_network.net_rtual.name
...
resource "azurerm_private_dns_zone" "dns_zone" {
  ...
  name                = "mysql.alfonso.mysql.database.azure.com"
  resource_group_name = azurerm_resource_group.test_cloudazure.name
}
...
resource "azurerm_private_dns_zone_virtual_network_link" "link_dns_zone" {
  ...
  name                  = "mysqlVnetZoneAlfonso.com"
  private_dns_zone_name = azurerm_private_dns_zone.dns_zone.name
  resource_group_name   = azurerm_resource_group.test_cloudazure.name
  virtual_network_id    = azurerm_virtual_network.net_rtual.id
}
...
resource "azurerm_mysql_flexible_server" "server_mysql" {
  ...
  name                         = "mysqlalfonso"
  resource_group_name          = azurerm_resource_group.test_cloudazure.name
  administrator_login          = "alfonso"
  administrator_password       = "Alfonso1234."
...
resource "azurerm_virtual_machine" "test_cloudazure" {
  ...
  os_profile {
    computer_name  = "Algonso"
    admin_username = "algonso"
    admin_password = "Algonso1234."
  }
